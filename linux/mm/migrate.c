				...
				struct list_head *ret)
{
  ...
  pr_info("unmap(): move_to_new_folio %d\n", rc);
	if (!page_mapped(hpage))
				    ...
				    int best_distance)
{
	...
	/*
	 * Extend to multi-tiered memory
	 * Set next demotion node to node + 1
	 * i.e. Local RAM next demotion node is Remote RAM
	 */
	if (node >= 0 && node < 3)
		nd->nodes[index] = (node + 1);
	else
		nd->nodes[index] = migration_target;
